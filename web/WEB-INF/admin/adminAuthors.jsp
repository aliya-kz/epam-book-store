...
<fmt:message bundle="${content}" key="AUTHOR_NOT_EXISTS" var="author_not_exists"/>
<fmt:message bundle="${content}" key="AUTHOR_EXISTS" var="author_exists"/>
<main>
    ...
    <section class="add-translation">
        ...
        <form action="/controller" method="post" onsubmit="return checkAuthorTranslation(this)">
            <table class="admin-table">
                <tr>
                    <th>ID</th>
                    <th><c:out value="${name}"/></th>
                ...
                <tr>
                    ...
                    <td><input type="hidden" name="service_name" value="add_author_translation">
                        <input type="submit" class="btn accept" value="${add_translation}"
                               onclick="checkAuthorTranslation()"></td>
        ...
        %>
        <c:choose>
            <c:when test="${msg eq 'author_exists'}">
                <div class="msg-div">
                    <c:out value="${author_exists}"/>
                </div>
            </c:when>
            <c:when test="${msg eq 'author_not_exists'}">
                <div class="msg-div">
                    <c:out value="${author_not_exists}"/>
                </div>
            </c:when>
        </c:choose>
    ...
    <section class="admin-filter">
        <input class="search-input" type="text" id="search-text" placeholder="${search}"
               onkeyup="tableSearch('admin-authors')">
    </section>
                ...
                <c:forEach var="author" items="${authors}">
                    <tr id="author${author.id}">
                        <td class="td-image"><a href="#" class="admin-image">
                            <img src="/image-servlet?image_id=${author.id}&table=authors" alt="author"
                                 width="90px"/></a></td>
