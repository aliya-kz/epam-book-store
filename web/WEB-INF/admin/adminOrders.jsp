        ...
        <table class="admin-table" id="admin-orders">
            <tr>
                ...
                <tr>
                                ...
                                <c:forEach var="stat" items="${statuses}">
                                    <c:choose>
                                        <c:when test="${stat.id == order.statusId}">
                                            <option value="${stat.id}" selected><c:out value="${stat.statusName}"></c:out> </option>
                                        </c:when>
                                        <c:otherwise>
                                            <option value="${stat.id}"><c:out value="${stat.statusName}"></c:out> </option>
                                        </c:otherwise>
                                    </c:choose>
                                </c:forEach>
                            </select>
                    ...
                    <td>
                        <button class="btn" id="view-cart" onclick="openForm('admin-order-books${order.id}')"><c:out value="${details}"/></button>
                    </td>
                </tr>
                        ...
                        <c:forEach var="entry" items="${order.orderItems}">
                            ...
                            <c:forEach var="book" items="${books}">
                                ...
                                <div class = "index-book">
                                    <img src = "/image-servlet?image_id=${book.id}&table=book_covers" alt = "${book.title}" width="80px"><br>
