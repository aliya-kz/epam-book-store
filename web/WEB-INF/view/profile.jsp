    ...
    <div class="bar">
      <ul class="bar-list">
        ...
        <li><form action = "<%= request.getContextPath()%>/controller" method = "post">
          ...
          <button class="fa-btn"> <i class="fas fa-sign-out-alt fa-2x"></i></button></form></li>
      </ul>
    </div>
  </section>
...
<main class="profile-main">
  <section id="prof-personal">
    ...
    <div class="profile-info">
      ...
      <form action = "<%= request.getContextPath()%>/controller?id=${user.id}" method = "post" onsubmit="return checkPersonal(this)">
        <button class="btn" id="save-profile" type="submit" name="service_name" value="edit_profile"
                onclick="checkPersonal()">${save}</button><br>
        ...
        <div class = "form-control">
          ...
          <input class="to-be-amended" style="display: none" id="surname" type = "text" placeholder="${surname}" name = "surname">
        ...
        <div class = "form-control">
          ...
          <input class="to-be-amended" style="display: none" id="name" type = "text" placeholder="${name}" name = "name">
          <i class = "fas-fa-check-circle"></i>
          <i class = "fas-fa-check-exclamation-circle"></i>
          <small>Error message</small>
        </div>
        ...
        <div class = "form-control">
          ...
          <input class="to-be-amended" style="display: none" id="phone" type = "text" placeholder="${phone}" name = "phone">
          <i class = "fas-fa-check-circle"></i>
          <i class = "fas-fa-check-exclamation-circle"></i>
          <small>Error message</small>
        </div>
      ...
      <c:forEach var="addr" items="${user.addresses}">
        <form action = "<%= request.getContextPath()%>/controller?id=${addr.id}&table=addresses" method = "post">
          ...
          <h2><i class="fas fa-map-marker-alt fa-lg"> </i><c:out value="${addr.address}"/>
            <button class="fa-btn"><i class="fas fa-trash-alt fa-lg"></i></button></h2>
        </form>
      </c:forEach><br>
      <form action = "<%= request.getContextPath()%>/controller" method = "post">
        <input type="text" name="address"/>
        <input type="hidden" name="uri" value="<%=request.getRequestURI()%>"/>
        <button class="fa-btn" name="service_name" value="add_address"><i class="fas fa-plus-square fa-lg"></i></button>
      </form>
    </div>
  </section>
  ...
  <section id="prof-change-password">
    ...
    <div class="profile-info">
      ...
      <form id="change-pass" action = "<%= request.getContextPath()%>/controller?id=${user.id}" onsubmit="return checkPassword(this)" method = "post">
        <div class = "form-control">
        ...
        <input type="hidden" name="uri" value="<%=request.getRequestURI()%>"/>
        <button class="btn" type="submit" onclick="checkPassword()" id="save-pass" name="service_name" value="change_password"><c:out value="${save}"/></button>
      </form>
          ...
          <td>
            <form  action = "<%= request.getContextPath()%>/controller?order=${order.id}" method="post">
              ...
              <c:if test="${order.statusId == 4}">
                <button class="btn accept" name="status" value="6"><c:out value="${refund}"></c:out> </button>
              </c:if>

              <c:if test="${order.statusId < 3}">
                <button class="btn decline" name="status" value="5"><c:out value="${cancel}"></c:out> </button>
              </c:if>
            </form>
          </td>
        </tr>
