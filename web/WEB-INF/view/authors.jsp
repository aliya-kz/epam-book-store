                    ...
                    <c:forEach var="author" items="${authors}">
                        <c:forEach var="authId" items="${found}">
                            <c:if test="${author.id == authId}">
                                <div class="authors-item">
                                    <img src="/image-servlet?image_id=${author.id}&table=authors"
                                         alt="${author.fullName}"
                                         style="max-height: 230px; max-width: 180px"><br>
                                    <a href="/author?id=${author.id}" class="index-book-name"><h2><c:out
                                            value="${author.fullName}"/></h2></a>
                                </div>
                            </c:if>
                        </c:forEach>
                    </c:forEach>
        ...
        <c:forEach var="author" items="${authors}">
            <div class="authors-item">
                <img src="/image-servlet?image_id=${author.id}&table=authors" alt="${author.fullName}"
                     style="max-height: 230px; max-width: 180px"><br>
                <a href="/author?id=${author.id}" class="index-book-name"><h2><c:out value="${author.fullName}"/></h2>
                </a>
            </div>
        </c:forEach>
    </section
