                ...
                <div class="index-book" onmouseover="showEl('book-add${book.id}')"
                    ...
                    <c:if test="${book.quantity < 1}">
                        <div id="book-add${book.id}" class="btn checkout"
                             style="margin-top: 5px; pointer-events:none; width: 170px">
                            <c:out value="${out_of_stock}"/></div>
                    </c:if>
                    <c:if test="${book.quantity > 0}">
                        <form id="book-add${book.id}" style="display:none"
                              onsubmit="showEl('grey-background${book.id}')"
                            ...
                            <input type="submit" class="btn"
                                   style="margin-top: 5px; width: 170px; background-color: #24575c"
                                   value="${add_to_cart}"/>
                        </form>
                    </c:if>
                </div>
                ...
                <div id="index-books">
                    <c:forEach var="book" items="${books}">
                        <c:if test="${book.categoryId == category.id}">
                            <div class="index-book" onmouseover="showEl('book${book.id}')"
                                 onmouseleave="hideEl('book${book.id}')">
                                <img src="/image-servlet?image_id=${book.id}&table=book_covers" alt="${book.title}"
                                     style="height: 230px; max-width: 180px"><br>
                                <a href="/book?id=${book.id}" class="index-book-name"><c:out value="${book.title}"/></a>
                                <c:set var="authorIds" value="${book.authors}"/>
                                ...
                                <c:if test="${book.quantity > 0}">
                                    <form id="book${book.id}" style="display:none"
                                          onsubmit="showEl('grey-background${book.id}')"
                                          action="<%=request.getContextPath()%>/controller?id=${book.id}&quantity=1"
                                          method="post">
                                        <input type="hidden" name="service_name" value="add_to_cart">
                                        <input type="hidden" name="uri" value="<%=request.getRequestURI()%>">
                                        <input type="submit" class="btn"
